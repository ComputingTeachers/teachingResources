...
In that case, it is silly to even check even numbers, as we know they will noe be prime, and so you will notice that I have adjusted out loop to suit.
...
This means that no number will be divisible (leaving no remainder) by any number bigger than it's half. And so in our for loop, instead of the condition being `i < n`, we can make it:

```
i < n/2
```

because anything higher than n/2 will not go into n.
